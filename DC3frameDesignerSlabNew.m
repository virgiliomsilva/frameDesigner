%% DC3frameDesigner
function [] = DC3frameDesignerSlabNew(buildingName, fck, fyk, cover, seismicVerticalLoadCase, seismicCases, nonSeismicCases, folder, flag, slabTopReinf)
...
tic; disp('Started DC3 slab new');
%% STEP 1 - GETTING DATA
loading = waitbar(0,'Reading data','Name', 'DC3 slab New: Step 1 of 9');
...
loading = waitbar(1,loading,'Reading data','Name', 'DC3 slab New: Step 1 of 9'); pause(.5);
%% STEP 2 - DESIGN BEAMS
close(loading); loading = waitbar(0,'Initializing beams','Name', 'DC3 slab New: Step 2 of 9'); pause(1);
% beams = []; beamsMid = [];
...
% close(loading); loading = waitbar(0,'Initializing columns','Name', 'DC3 slab New: Step 3 of 9'); pause(1);
...
%     
%     count = count + 1;
%     loading = waitbar(count / size(barsOfColumns,1),loading,'Columns progress','Name', 'DC3 slab New: Step 3 of 9');
% end
% 
...
% close(loading); loading = waitbar(0,'Initializing columns resisting bending moments update','Name', 'DC3 slab New: Step 4 of 9'); pause(1);
...
%     
%     loading = waitbar(i / size(columns,1),loading,'Columns resisting bending moments update progress','Name', 'DC3 slab New: Step 4 of 9');
% end
% clear areaRebar b h i j M_Rd mAux N_Axial
% %% STEP 5 - DO THE RESISTING BENDING MOMENTS [1.3] COMPARISON
% close(loading); loading = waitbar(0,'Initializing bending comparisons','Name', 'DC3 slab New: Step 5 of 9'); pause(1);
% increNeed = [];
...
% close(loading); loading = waitbar(0,'Initializing column re-reinforcement','Name', 'DC3 slab New: Step 6 of 9'); pause(1);
...
%     
%     loading = waitbar(i / (size(barsOfColumns,1)),loading,'Column re-reinforcement progress','Name', 'DC3 slab New: Step 6 of 9');
%     clear primeColumns
%     newColumns = [];
% end
...
%%
load('D:\Google Drive\Disco UPorto\MIECIVIL\5 Ano\2 Semestre\MatlabFiles\frameDesigner\output\regular_DC3slabNew\DC3columnsIt2rule13.mat')
load('D:\Google Drive\Disco UPorto\MIECIVIL\5 Ano\2 Semestre\MatlabFiles\frameDesigner\output\regular_DC3slabNew\DC3beamsIt1.mat')
%%
%% STEP 7 - SHEAR CAPACITY DESIGN BEAMS
close(loading); loading = waitbar(0,'Initializing Beam seismic equilibrium','Name', 'DC3 slab New: Step 7 of 9'); pause(1);
...
for i = 1 : length(beamDesiOrd)
    ...
    loading = waitbar(i / (length(beamDesiOrd)),loading,'Beam seismic equilibrium','Name', 'DC3 slab New: Step 7 of 9');
end
...
close(loading); loading = waitbar(0,'Initializing column seismic equilibrium','Name', 'DC3 slab New: Step 8 of 9'); pause(1);
...
for i = 1 : length(pilars)
    ...
    loading = waitbar(i / (length(pilars)),loading,'Column seismic equilibrium progress','Name', 'DC3 slab New: Step 8 of 9');
end
...
close(loading); loading = waitbar(0,'Updating and writing to Seismo','Name', 'DC3 slab New: Step 9 of 9'); pause(1);
toSeismo(seismicColumns(:,[1:5, 9:11]), seismicBeams(:,[1:5, 8:10]), nodes, element, stories, fck, fyk, cover, folder);
loading = waitbar(1, loading,'Updating and writing to Seismo','Name', 'DC3 slab New: Step 9 of 9');
%% FINISH
