...
% Given a bar (column) name, will find the minimum width possible 
% compatible with the adjacent bars (beams)
function [minWidth] = minWidFind(barName, element, beams)
    nodes = element(element(:,1) == barName,[2 3]);
    bars = [];
    for i = 2 : 3
        for j = 1 : size(element,1)
            if (element(j, i) == nodes(1) || element(j, i) == nodes(2)) && element(j,4) ~= 3
