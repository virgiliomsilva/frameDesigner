function [sec_h, sec_b, longReinfNo, longReinfPhi, longReinfArea, M_Rd, roMinCondition, shearReinfPhi, shearReinfSpac, shearReinfLoops, V_Rd, sCondition] = DC3beamDesign(fck, fyk , cover, M_Ed, Fz_Ed, given_b, given_h, longReinfN, longReinfPh)
    %% INFO SELECTION
    ...
    dbw = 8;
    [max_spacing, sCondition8]= min([.75 * d, sec_h/4, 24*dbw, 8*dbl]);
    shearReinforce(shearReinforce(:,1) == dbw & shearReinforce(:,3) > max_spacing,:) = [];
    dbw = 10;
    [max_spacing, sCondition10]= min([.75 * d, sec_h/4, 24*dbw, 8*dbl]);
    shearReinforce(shearReinforce(:,1) == dbw & shearReinforce(:,3) > max_spacing,:) = [];
