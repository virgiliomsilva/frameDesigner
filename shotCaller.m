...
% visdiff('DC1columnDesign.m','DC1columnDesignIts.m')
...
global fck fcd fctm fyk fyd fywd cover dMax incr

fck   = 30;             %GPa
fcd   = fck / 1.5;      %GPa
fctm  = 0.3 * fck^(2/3);%GPa
fyk   = 400;            %GPa
fyd   = fyk / 1.15;     %GPa
fywd  = fyd;            %GPa
cover = 0.035;          %m
dMax  = 0.03;           %dimensão máxima do agregado
incr  = 0.05;           %incremental steps 
...
mkdir(folder);
DC1frameDesigner(buildingName1, seismicVerticalLoadCase, nonSeismicCases, folder);
%% DC2
