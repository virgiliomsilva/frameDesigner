...
% four sides
% function [sec_h, sec_b, noRebar, phiRebar, areaRebar, Ro, M_Rd, shearReinfPhi, shearReinfSpac, shearReinfLoops, shearReinfArea, V_Rd, sCondition] = DC1columnDesign(fck, fyk , cover, N_Axial, My_h, Mz_b, V_Ed, givenWidth, givenLong)
function [sec_h, sec_b, noRebar, phiRebar, areaRebar, Ro, M_Rd, shearReinfPhi, shearReinfSpac, shearReinfLoops, shearReinfArea, V_Rd, sCondition] = DC1columnDesign(N_Axial, My_h, Mz_b, V_Ed, givenWidth, givenLong)
sCondition = 0;
%% INFO SELECTION
global fck fcd  fyk fyd fywd dMax cover incr
...
My_h = abs(My_h); Mz_b = abs(Mz_b);
%% DIMENSIONS & LONG REBAR
if  isa(N_Axial, 'double') % == 0 && My_h == 0 && Mz_b == 0
...
if isa(V_Ed, 'double')
    ...
    if ~isa(N_Axial, 'double')
       b = givenWidth;
       phiRebar = givenLong(1, 1);
       noRebar  = givenLong(1, 2);
    end
